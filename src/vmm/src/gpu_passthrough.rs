...
use kvm_bindings::{
    kvm_irq_routing, kvm_irq_routing_entry, kvm_irqfd, KVM_IRQFD_FLAG_RESAMPLE,
...
impl GpuPassthroughManager {
    ...
    fn get_current_routes(&self, vm_fd: &VmFd) -> io::Result<Vec<kvm_irq_routing_entry>> {
        ...
        loop {
            ...
            let ret = unsafe { ioctl(vm_fd.as_raw_fd(), KVM_GET_GSI_ROUTING, routing_ptr) };
            ...
            if ret >= 0 {
                ...
                println!(
                    "KVM Routing: Read {} existing entries from kernel.",
                    actual_nr
                );
    ...
    fn setup_irq_routing(&self, vm_fd: &VmFd) -> io::Result<()> {
        ...
        println!(
            ...
            original_count
                - entries
                    .iter()
                    .filter(|e| self.irqs.iter().any(|ctx| ctx.gsi == e.gsi))
                    .count(), // 简单估算
            self.irqs.len()
        );
        ...
        // 4. 写入全量路由表
        let ret = unsafe { ioctl(vm_fd.as_raw_fd(), KVM_SET_GSI_ROUTING, buffer.as_ptr()) };
    ...
    pub fn attach_to_kvm(&self, vm_fd: &VmFd) -> io::Result<()> {
        ...
        for (i, &index) in indices.iter().enumerate() {
            ...
            let ret = unsafe { ioctl(vm_fd.as_raw_fd(), KVM_IRQFD, &irqfd_data) };
            ...
            self.pmthor
                .set_unmask_event(index, ctx.resample.as_raw_fd())?;
            println!(
